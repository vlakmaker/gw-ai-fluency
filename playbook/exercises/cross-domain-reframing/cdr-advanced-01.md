---
pillar: cross-domain-reframing
level: advanced
archetype-fit: [connector, innovator]
time-estimate: 40min
tags: [exercise, pillar/cross-domain-reframing, level/advanced, type/exercise]
---

# CDR-Advanced-01

> **One-liner:** <!-- What you'll walk away being able to do after this exercise. -->

---

## 🔧 Jump in (Tinkerers start here)

<!-- The challenge itself. Minimal preamble. Clear action: what to do, with what tool, on what input. Include a concrete starter prompt or example. -->

---

## 📋 Plan first (Planners start here)

<!-- A short breakdown of what you're about to do and why, in 3–5 steps. -->

---

## 🧭 Why this matters (Strategists start here)

<!-- The strategic context: what capability this builds, where it fits in the bigger picture. 2–3 sentences max. -->

---

## Reflection

- What surprised you about the output?
- What did you have to fix or override?
- How would you explain what you just did to a colleague?
- 💬 *Discuss: Try explaining your result to someone who hasn't used AI for this task. What questions do they ask?* (Social Learners)

## ⬆️ Level up

You've reached the advanced level for Cross-Domain Reframing. Consider exploring advanced exercises in other pillars, or revisit this exercise to deepen your approach.

Back to [[pillars/cross-domain-reframing|Cross-Domain Reframing]] | [[levels/advanced|🔴 Advanced Level]]
